...
# Permalink normalizations (required)
# /chat\/(.*?)\/page-\d+/chat/\1

class ImportScripts::SimplePress < ImportScripts::Base

  SIMPLE_PRESS_DB ||= ENV['SIMPLEPRESS_DB'] || "local_cm_simplepress"
  TABLE_PREFIX = "wp_sf"
  BATCH_SIZE ||= 1000
  ...
  def import_categories
    ...
    categories = mysql_query(<<-SQL
        SELECT forum_id, forum_name, forum_seq, forum_desc, parent, forum_slug
    ...
    create_categories(categories) do |c|
      ...
      url_slug = "chat/#{c["forum_slug"]}"
      Permalink.create(url: url_slug, category_id: category_id_from_imported_category_id(c['forum_id']))
    ...
    batches(BATCH_SIZE) do |offset|
      results = mysql_query("
        SELECT p.post_id id,
          ...
          JOIN #{TABLE_PREFIX}forums f ON (t.forum_id = f.forum_id)
      ...
      results.each do |result|
        topic = topic_lookup_from_imported_post_id(result['id'].to_i)
        if topic.present?
          url_slug = "chat/#{result["forum_slug"]}/#{result["slug"]}"
